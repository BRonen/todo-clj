...
The `home-routes` handler in the `todo-clj.routes.home` namespace
...
The `home-page` function will in turn call the `todo-clj.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `todo-clj.handler` namespace:
...
Request middleware functions are located under the `todo-clj.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `todo-clj.dev-middleware` namespace found in
